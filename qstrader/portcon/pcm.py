...
class PortfolioConstructionModel(object):
    ...
    def _obtain_full_asset_list(self, dt: pd.Timestamp) -> list[str]:
        """
    ...
    def _create_full_asset_weight_vector(
        self, zero_weights: dict[str, float], optimised_weights: dict[str, float]
    ) -> dict[str, float]:
        """
    ...
    def __call__(self, dt: pd.Timestamp, stats: dict = None) -> list[Order]:
        """
        ...
        if self.alpha_model:
            weights: dict[str, float] = self.alpha_model(dt)
        else:
        ...
        optimised_weights: dict[str, float] = self.optimiser(
            dt, initial_weights=weights
        )

        # Ensure any Assets in the Broker Portfolio are sold out if
        # they are not specifically referenced on the optimised weights
        full_assets: list[str] = self._obtain_full_asset_list(dt)
        full_zero_weights: dict[str, float] = self._create_zero_target_weight_vector(
            full_assets
        )
        full_weights: dict[str, float] = self._create_full_asset_weight_vector(
