...
class QuantTradingSystem(object):
    ...
    def _create_order_sizer(self, **kwargs) -> OrderSizer:
        ...
        if self.long_only:
            ...
            order_sizer: OrderSizer = DollarWeightedCashBufferedOrderSizer(
                self.broker,
            ...
            order_sizer: OrderSizer = LongShortLeveragedOrderSizer(
                self.broker,
                self.broker_portfolio_id,
                self.data_handler,
    ...
    def _initialise_models(self, **kwargs) -> None:
        ...
        order_sizer: OrderSizer = self._create_order_sizer(**kwargs)

        # TODO: Allow optimiser to be generated from config
        optimiser: FixedWeightPortfolioOptimiser = FixedWeightPortfolioOptimiser(
        ...
        # Execution
        execution_algo: MarketOrderExecutionAlgorithm = MarketOrderExecutionAlgorithm()
    ...
    def __call__(self, dt, stats=None):
        ...
        rebalance_orders: list[Order] = self.portfolio_construction_model(
            dt, stats=stats
        )
